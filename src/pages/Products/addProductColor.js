...
const AddProductColor = () => {
    ...
    const addProductColor = (e) => {
        ...
        if(formFields.productColor===""  ){
            context.setAlertBox({
                open:true,
                error: true,
                msg: "Vui lòng thêm Color"
            });
            return false;
        }
        setIsLoading(true);
        ...
        if(editId==""){
            postData('/api/productColor/create', formFields).then((res)=>{
                context.setAlertBox({
                    open:true,
                    error: false,
                    msg: "Them Color thanh cong"
                });
                setFormFields({
                    productColor: "",
                })
            ...
            editData(`/api/productColor/${editId}`, formFields).then((res)=>{
                fetchDataFromApi("/api/productColor").then((res)=>{
                    setProductColorData(res);
                    setEditId("");
                    setIsLoading(false)
    ...
    const updateData=(id)=>{
        fetchDataFromApi(`/api/productColor/${id}`).then((res)=>{
            setEditId(id);
            setFormFields({
    ...
    return(
        <div className="right-content w-100">
                <div className="card shadow border-0 w-100 flex-row p-4">
                    <h5 className="mb-0">Thêm Color</h5>
                ...
                <form className='form' onSubmit={addProductColor} >
                    <div className='row'>
                        <div className='col-sm-8'>
                            <div className='card p-4 mt-0'>
                                <div className='row'>
                                    <div className='col'>
                                        <div className='form-group'>
                                            <h6>Mau sản phẩm</h6>
                                            <input type='text' name='productColor' value={formFields.productColor} onChange={inputChange} />
                                        </div>
                                    </div>
                                </div>
                                ...
                                <Button type="submit" className='btn-blue btn-lg btn-big w-100'>
                                    ...
                                    <CircularProgress color="inherit" className=' loader' /> : 'Thêm màu'}
                                </Button>
                            </div>
                        </div>
                    </div>
